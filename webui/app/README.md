...
```sh
npm run build      # 产物输出到 dist/（后续 rust-embed 进 nipa-server）
...
src/
  ...
  lib/
    ...
    nav.svelte.ts       # hash 路由（#/library #/steward #/console #/settings）
    format.ts           # 格式化 & art-g1..g7 海报占位分配
  components/
    AgentTimeline.svelte    # 事件流 timeline 渲染器（SSE 实时 / 回放同构）
    ItemDetailModal.svelte  # 条目详情浮层
  views/
    LibraryView.svelte      # 媒体库（Hero + 海报墙 + 扫描占位卡）
